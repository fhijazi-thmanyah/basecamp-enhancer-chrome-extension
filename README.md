...
Both run continuously: a `MutationObserver` enhances new content as Basecamp streams it in, and every operation is **idempotent** — re-running never produces duplicate badges or re-sets attributes.

## Toggles

Click the toolbar icon for a popup that enables/disables each feature independently. Changes apply **live** to open Basecamp tabs (no reload) and persist via `chrome.storage.sync`. With both toggles off, the extension fully reverts its changes — plain Basecamp.
