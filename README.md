...
5. **Claude Code launcher** — a floating button (bottom-right, always reachable) that spawns a **Claude Code worker on the local HQ server** (`127.0.0.1:8377`) to handle or watch the current conversation. Click it, type a prompt, pick a cadence (**One-shot / 15 min / 60 min**), hit Launch — the worker gets your prompt + this page's URL + a fixed watch template, runs in `career-coach`, and the popover confirms whether claude **actually started** (HQ's spawn 200 alone doesn't guarantee that) and shows a live-status tray of your launched sessions with a link to the HQ dashboard. Requires the HQ server to be running locally; only the typed prompt and the URL are sent — never page content.
