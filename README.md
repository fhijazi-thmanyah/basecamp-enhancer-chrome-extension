...
4. **Inline message menu** — the entire chat-line "…" menu (**Edit, Reply, Bookmark, Bubble up, Copy link, Delete**) lifted **inline beside each bubble**, so you never open the kebab. It's Basecamp's own menu (its controllers reconnect, so every action works natively) — your messages get the menu on the left, everyone else's on the right. Loads lazily as messages scroll into view.
