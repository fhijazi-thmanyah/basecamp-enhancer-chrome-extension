...
3. **Quick reactions** — one-click boost emoji so you react without opening the "…" picker. The set is **fixed and yours to arrange**: it shows exactly the emoji you configure in the popup, in the order you set them (no automatic recently-used reshuffling). On chat/pings and comments the emoji live in the hover bar (below); on the main card/message/todo detail they sit inline next to Basecamp's "+".
...
5. **Claude Code launcher** — a Claude Code button on **every open conversation** (the main card/todo/message view *and* each chat/ping window get their **own** button — a sidebar ping and the main card launch independently, each with its pane's URL). In chat/ping windows it sits **in the composer's tool row** (after the format button); on the main view it's a bookmark-style tab in the left gutter that follows the scroll and clamps to the top so it's always visible. It stays put while the convo scrolls, **spins while a worker you launched there is actively working**, and the popover closes on Esc or a click anywhere outside. Click it, type a prompt — **or leave it empty to "decide and respond automatically"** (the worker reads the thread and responds/acts as needed, never doing anything destructive) — pick a **Watch ⏱️** cadence (**One-shot / 15 min / 60 min**; **Enter launches, Shift+Enter for a newline**, Esc closes), optionally tick **"Reply when done"** (the worker replies to the thread and @-mentions the relevant people when it finishes) — a **Claude Code worker is spawned on the local HQ server** (`127.0.0.1:8377`) with your prompt + that conversation's URL + a fixed watch template, running in `career-coach`. The popover confirms whether claude **actually started** (HQ's spawn 200 alone doesn't guarantee that) and shows a live-status tray of your launched sessions with a link to the HQ dashboard. Requires the HQ server to be running locally; only the typed prompt and the URL are sent — never page content.
