...
(() => {
  ...
  // ---- Telemetry (PostHog) ----------------------------------------------
  // Usage + error analytics identified by the signed-in Basecamp user, so
  // error reports say WHO hit them and on WHICH page. The real posthog-js SDK
  // is BUNDLED at vendor/posthog.js (the `array.full.no-external.js` build:
  // every lazy dependency inlined, zero runtime CDN loading — MV3 forbids
  // remote code) and listed before this file in manifest.json, so `posthog`
  ...
  // Make the SDK state match settings.telemetry. Called once storage has
  // resolved and again on every settings change (live toggle, no reload).
  function phApply() {
    if (!PH_KEY || typeof posthog === "undefined") return;
    if (!phStarted && settings.telemetry) {
      phStarted = true;
      posthog.init(PH_KEY, {
        api_host: PH_HOST,
    ...
    if (phStarted) {
      // guard on current state: opt_in_capturing() unconditionally would fire
      // its own event on every call
      if (settings.telemetry && posthog.has_opted_out_capturing()) {
        posthog.opt_in_capturing({ captureEventName: null });
      } else if (!settings.telemetry && !posthog.has_opted_out_capturing()) {
        posthog.opt_out_capturing();
      }
    }
  }

  let phWho = null; // {id, email, name} — resolved once, cached in storage.local

  async function phIdentity() {
    if (phWho) return phWho;
    const pid = (document.querySelector('meta[name="current-person-id"]') || {}).content || "";
    try {
      const st = await chrome.storage.local.get("bceWho");
      if (st.bceWho && st.bceWho.id === pid && st.bceWho.email) return (phWho = st.bceWho);
    } catch { /* storage gone (orphaned) — fall through to a fresh lookup */ }
    // The signed-in user's email isn't in the page DOM; their profile page has
    // it. One same-origin fetch, then cached for good.
    let email = "", name = "";
    try {
      const link = document.querySelector('a[href*="/my/profile"]');
      if (link) {
        const html = await (await fetch(link.href)).text();
        email = (html.match(/[\w.+-]+@[\w-]+(?:\.[\w-]+)+/) || [""])[0];
        name = ((html.match(/<h1[^>]*>\s*([^<]+?)\s*</) || [])[1] || "").trim();
      }
    } catch { /* offline or markup change — identify by person id instead */ }
    phWho = { id: pid, email, name };
    try { chrome.storage.local.set({ bceWho: phWho }); } catch { /* orphaned */ }
    return phWho;
  }

  function tele(event, props = {}) {
    if (phStarted && settings.telemetry) posthog.capture(event, props);
  }

  // Our own crashes (and only ours): both listeners filter on the extension's
  // chrome-extension:// URL appearing in the source/stack, so Basecamp's own
  // script errors never get reported.
  const phExtUrl = (() => { try { return chrome.runtime.getURL(""); } catch { return "\u0000"; } })();
  window.addEventListener("error", (e) => {
    const src = (e.filename || "") + ((e.error && e.error.stack) || "");
    if (!src.includes(phExtUrl)) return;
    tele("extension_error", { message: String(e.message).slice(0, 500),
                              source: e.filename, line: e.lineno });
  });
  window.addEventListener("unhandledrejection", (e) => {
    const stack = (e.reason && e.reason.stack) || "";
    if (!stack.includes(phExtUrl)) return;
    tele("extension_error", { message: String((e.reason && e.reason.message) || e.reason).slice(0, 500),
  ...
  async function sendBoost(container, emoji, btn) {
    ...
    try {
      ...
      tele("reaction_sent", { emoji, ok: res.ok, status: res.status });
    } catch (e) {
      // network hiccup — the button re-enables and the user can retry
      tele("extension_error", { where: "sendBoost", message: String(e).slice(0, 500) });
    } finally {
      btn.disabled = false;
    }
  }
  ...
  function ccBuildPopover(url) {
    ...
    launch.addEventListener("click", async () => {
      ...
      tele("cc_launch", { ok: r.ok, loop, auto: !typed, error: r.ok ? undefined : r.error });
      if (!r.ok) {
      ...
      tele("cc_launch_result", { ok: verdict.ok,
                                 outcome: verdict.ok ? verdict.worker.status : verdict.reason });
      if (verdict.ok) {
  ...
  // A font picker (see DEFAULTS.bcFont): styles.css maps data-bce-font="<choice>"
  // on <html> to a family via a CSS variable, and the @font-faces are injected
  // here at runtime from chrome.runtime.getURL — NOT hardcoded in styles.css.
  // The scheme differs per browser (chrome-extension:// vs safari-web-extension://),
  // and Safari doesn't substitute __MSG_@@extension_id__ in content-script CSS,
  // so a URL built at runtime is the only form that works in both. Faces still
  // load lazily: the browser only fetches a face once a rule uses its family.

  const FONT_FACES = [
    ["IBM Plex Sans Arabic", "IBMPlexSansArabic-Light.woff2", "300"],
    ...
    ["IBM Plex Sans Arabic", "IBMPlexSansArabic-Bold.woff2", "700 900"],
    ["Thmanyah Sans", "thmanyahsans-Regular.woff2", "300 400"],
    ["Thmanyah Sans", "thmanyahsans-Medium.woff2", "500 600"],
    ["Thmanyah Sans", "thmanyahsans-Bold.woff2", "700 900"],
    ["Thmanyah Serif Text", "thmanyahseriftext-Regular.woff2", "300 400"],
    ["Thmanyah Serif Text", "thmanyahseriftext-Medium.woff2", "500 600"],
    ["Thmanyah Serif Text", "thmanyahseriftext-Bold.woff2", "700 900"],
    ["Thmanyah Serif Display", "thmanyahserifdisplay-Regular.woff2", "300 400"],
    ["Thmanyah Serif Display", "thmanyahserifdisplay-Medium.woff2", "500 600"],
    ["Thmanyah Serif Display", "thmanyahserifdisplay-Bold.woff2", "700 900"],
  ];

  function ensureFontFaces() {
    if (document.getElementById("bce-fontfaces") || !ctxAlive()) return;
    const css = FONT_FACES.map(([fam, file, w]) =>
      `@font-face{font-family:"${fam}";src:url("${chrome.runtime.getURL("fonts/" + file)}") format("woff2");font-weight:${w};}`
    ).join("\n");
